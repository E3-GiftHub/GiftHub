...
async function createItemCatalogue(retailers) {
    const items = [];
    const itemsData = [
...
async function createInvitations(users, events) {
    const invitations = [];
    for (const event of events) {
        ...
        for (const user of users) {
            if (user.username !== creator.username) {
                let status;
                if (Math.random() > 0.3) {
                    status = client_1.Status.ACCEPTED;
                }
                else {
                    const isPending = Math.random() > 0.5;
                    status = isPending ? client_1.Status.PENDING : client_1.Status.REJECTED;
                }
                const repliedAt = status !== client_1.Status.PENDING ? new Date() : null;
...
async function createEventArticles(events, items) {
    const eventArticles = [];
    for (const event of events) {
        ...
        for (const item of selectedItems) {
            ...
            const priorities = [client_1.PriorityType.LOW, client_1.PriorityType.MEDIUM, client_1.PriorityType.HIGH];
...
async function createMarks(users, events, items) {
    const marks = [];
    const markTypes = [client_1.MarkType.PURCHASED, client_1.MarkType.RESERVED];
    for (const event of events) {
        const eventArticles = await prisma.eventArticle.findMany({
            where: { eventId: event.id },
        });
        const invitations = await prisma.invitation.findMany({
            where: { eventId: event.id, status: client_1.Status.ACCEPTED },
        });
...
async function createContributions(users, events, items) {
    const contributions = [];
    for (const event of events) {
        const invitations = await prisma.invitation.findMany({
            where: { eventId: event.id, status: client_1.Status.ACCEPTED },
        ...
        if (eventArticles.length === 0 || invitedUsers.length === 0)
            continue;
                    ...
                    if (amount > 0) {
                        ...
                        await prisma.mark.create({
                            data: {
                                ...
                                eventId: event.id,
                                articleId: itemId,
                                type: client_1.MarkType.CONTRIBUTED,
                            },
                        });
...
async function createMedia(users, events) {
    const media = [];
...
async function createUserReports(users) {
    const reports = [];
...
async function createEventReports(users, events) {
    const reports = [];
